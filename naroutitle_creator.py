...
def train(model, critation, input, target,criterion):
...
def train_iter(train_data, modelsave,e_dim=256,h_dim=256):
    titles = []
    ...
    for iter in range(1, n_iters + 1):
        ...
        for i, name_idx in enumerate(names_idx):
            ...
            loss = train(model, criterion, inputa, target,criterion)
            total_loss += loss
        ...
        if best_loss > float(total_loss / len(names_idx)):
            ...
            no_improve_iter = 0
            torch.save(model.state_dict(), modelsave + "_{:06}.model".format(iter))
        else:
            no_improve_iter += 1
...
if __name__ == '__main__':
    # train_iter('2ch_scraped_list.txt', './models/2ch_char')
    train_iter('titledata.txt', './models/word_narou_char')
